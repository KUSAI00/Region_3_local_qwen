...
class SmolVLM2Model():
    def __init__(self,
        ...
        """
        Initialize the local SmolVLM2 model with 4-bit quantization.
                ...
                for content_item in msg["content"]:
                    if content_item["type"] == "image_url":
                        ...
                        if url.startswith("data:image"):
                            ...
                            # fallback for plain base64 or other formats if any
                            image = Image.open(io.BytesIO(base64.b64decode(url))).convert("RGB")
                        images.append(image)
                        formatted_msg["content"].append({"type": "image"})
                    else:
    ...
    def ground(self, instruction, image):
        ...
        try:
            ...
            elif '<point>' in response:
                coords = response.split('<point>')[1].split('</point>')[0]
                coords = json.loads(coords)
                point = [coords[0]/1000.0, coords[1]/1000.0]
            else:
                coords = pred_2_point(response)
                if coords:
                    point = [coords[0]/1000.0, coords[1]/1000.0]
            ...
            if point is not None:
                ...
                raise ValueError("Could not extract point")
        except:
            result_dict = {
                "result": "wrong_format",
                "format": "x1y1x2y2",
    ...
    def region_focus(self, instruction, image, debug=False, task_id=None, temperature=0, top_p=1.0, system_message=None):
        ...
        try:
            if '<tool_call>\n' in response:
                ...
                coords = response.split('<point>')[1].split('</point>')[0]
                coords = json.loads(coords)
                return [coords[0]/1000.0, coords[1]/1000.0], response
            else:
                coords = pred_2_point(response)
                if coords:
                    return [coords[0]/1000.0, coords[1]/1000.0], response
    ...
    def next_action_regionfocus(self, instruction, zoomed_img_bytes, left, top, zoom_x, zoom_y,
        ...
        try:
            if '<tool_call>\n' in response:
                ...
                coords = response.split('<point>')[1].split('</point>')[0]
                coords = json.loads(coords)
                click_point = [coords[0], coords[1]] # Assume these might need scaling later
            else:
                coords = pred_2_point(response)
                if coords:
                    click_point = [coords[0], coords[1]]
        except:
            return None, response
        ...
        if click_point:
            ...
            if '<point>' in response or not ('<tool_call>\n' in response):
